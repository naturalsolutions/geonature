...
* ``02_configure_systemd.sh`` : Création des services systemd ``geonature``, ``geonature-worker`` et ``geonature-beat``, configuration de ``logrotate``, création des dossiers ``/run/geonature`` et ``/var/log/geonature``.
