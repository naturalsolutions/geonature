...
- ``observations`` : liste des observations créées / modifiées

.. note::

   Pour chaque utilisateur, la liste des observations est filtrée en fonction de ses permissions de lecture de la synthèse.
   Dans le cas où il n’a accès à aucune des observations ayant été créées / modifiées, la notification n’est pas envoyée.
