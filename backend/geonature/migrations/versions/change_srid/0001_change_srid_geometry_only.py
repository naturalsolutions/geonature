...
revision = "0001_change_srid_geometry_only"
down_revision = None
branch_labels = ("change_srid",)
depends_on = None

SCRIPTS_SQL_PATH_UPGRADE = [
    "../../data/custom_changes/00_prepare_srids.sql",
    "../../data/custom_changes/01_drop_constraints.sql",
    "../../data/custom_changes/02_drop_index.sql",
...
def _detect_current_srid(bind) -> int:
...
def upgrade():
    ...
    print(f"🗺️ Utilisation du SRID = {srid}")
    OLD_SRID = _detect_current_srid(bind)
    logger.info("=== Migration SRID - Partie 1 : géométries ===")
    with logging_redirect_tqdm():
        _run_sql_files_with_progress(
            bind, SCRIPTS_SQL_PATH_UPGRADE, {"old_srid": OLD_SRID, "new_srid": srid}
        )
    logger.info("=== Fin migration géométries ===")


def downgrade():
    bind = op.get_bind()
    srid = get_input_srid(bind)
    logger.info("=== Finalisation SRID après import raster ===")
    with logging_redirect_tqdm():
        _run_sql_files_with_progress(bind, SCRIPTS_SQL_PATH_DOWNGRADE, {"new_srid": srid})
    logger.info("=== Fin finalisation SRID ===")
